...
const Viewevents = () => {
    ...
    const singleEvent = useSelector((state) => state.misc.miscellaneousdata.singleEvent);
    ...
    const paymentReceivedSummary = singleEvent?.result?.payment_received_summary || {};
    ...
    const columns = [
        ...
        ...(subeventchoice === 'yes' ? [{ id: 'subEventSurname', header: 'Category', accessorKey: 'subEventSurname' }] : []),
        ...(hasPayment === 'yes' ? [{ id: 'accountNumber', header: 'Account Number', accessorKey: 'accountNumber' }] : []),
        { id: 'surname', header: 'Surname', accessorKey: 'surname' },
        { id: 'firstname', header: 'Firstname', accessorKey: 'firstname' },
        ...(hasPayment === 'yes' ? [{ id: 'amount', header: 'Amount', accessorKey: 'amount' }] : []),
        ...(generalPayment === 'no' ? [{ id: 'paymentCategory', header: 'Payment Category', accessorKey: 'paymentCategory' }] : []),
    ...
    const paymentColumns = [
        ...
        { id: 'accountNumber', header: 'Account Number', accessorKey: 'account_number' },
        { id: 'amount', header: 'Amount', accessorKey: 'amount' },
        { id: 'amountPaid', header: 'Amount Paid', accessorKey: 'amount_paid' },
        { id: 'paymentStatus', header: 'Payment Status', accessorKey: 'payment_status' },
        { id: 'paymentDate', header: 'Date of Payment', accessorKey: 'payment_date' },
    ];
    ...
    const data = responses.map((response, index) => {
        ...
        const rowData = {
            ...
            surname,
            firstname,
            amount: hasPayment === 'yes' ? (isNaN(response.amount) ? response.amount : Number(response.amount).toLocaleString()) : '',
    ...
    const paymentData = paymentReceived.map((payment, index) => ({
        ...
        amount: isNaN(payment.amount) ? payment.amount : Number(payment.amount).toLocaleString(),
        amount_paid: isNaN(payment.amount_paid) ? payment.amount_paid : Number(payment.amount_paid).toLocaleString(),
                        ...
                        <section className="text-gray-600 body-font">
                            <div className="container px-5 py-9 mx-auto flex items-center md:flex-row flex-col">
                                <div className="flex flex-col md:pr-10 md:mb-0 mb-6 pr-0 w-full md:w-auto md:text-left text-center">
                                    <h1 className="md:text-3xl text-2xl font-medium title-font text-zippy mb-4">{singleEvent?.result?.event_details?.event_name}</h1>
                                    <h2 className="text-sm text-black font-extrabold title-font mb-1 text-black">Event Start Date: {singleEvent?.result?.event_details?.event_start_date}</h2>
                                    <h2 className="text-sm text-black font-extrabold title-font mb-1 text-black">Event End Date: {singleEvent?.result?.event_details?.event_end_date}</h2>
                                    <h2 className="text-sm text-black font-extrabold title-font mb-1">Event Link:</h2>
                                </div>
                                <div className="flex md:ml-auto md:mr-0 mx-auto items-center flex-shrink-0 space-x-4">
                                    <button className="bg-green-200 inline-flex py-3 px-5 rounded-lg items-center hover:bg-gray-200 focus:outline-none">
                                        <span className="title-font font-medium">Register User</span>
                                    </button>
                            ...
                            <>
                                <h2 id="accordion-collapse-heading-2">
                                    ...
                                    >
                                        ...
                                        <svg
                                            data-accordion-icon=""
                                            className={`w-3 h-3 ${openPanel === 2 ? 'rotate-180' : ''}`}
                                            aria-hidden="true"
                                            xmlns="http://www.w3.org/2000/svg"
                                            fill="none"
                                            viewBox="0 0 10 6"
